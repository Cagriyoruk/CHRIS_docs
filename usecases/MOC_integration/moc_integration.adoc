...
. xref:#power9-cluster-ongoing-progress[Power9 Cluster]
. xref:#feedback-and-future-work[Feedback and Future Work]
...
Create a folder /tmp/small & add some files above 100KB to that folder first. Then run the below script to run a job. A simple dataset of `.mgz` files can be found here -> https://github.com/FNNDSC/mgz_converter_dataset[mgz_converter_dataset].
...
=== X86 Progress and Updates
    ...
    . The only issue that I've and the student teams encountered is the docker pulling limit. This still needs some attention considering more and more people are joining the project. Creating a pull secret doesn't work since the service accounts `builder` and `deployer` are using the default kubernetes/dockercfg secret. There should be a way to use docker credentials to modify the `builder` and the `deployer` so that we can't prevent having the pulling limit at some certain point.
